...
def is_toc_page(pdf_page):
    return len([x for x in re.split('(\d{1,}[^\.][– ]?[\d]?)\n', pdf_page) if x and x[0].isdigit()]) >= MIN_SEC_PER_PAGE and pdf_page.count(';') < 20
...
def find_toc(pdf):
    ...
    for i in range(MIN_TOC_PAGE, MAX_TOC_PAGE):
        if is_toc_page(pdf.get_page_text(i)):
            toc_page_list = get_toc_pages(pdf.get_page_text(i, sort=SORT_NEEDS[pdf.name]))
            # fixed_lines = fix_lines(pdf.get_page_text(i).split('\n'))
...
def toc_tests():
    ...
    assert(find_toc(pdf02) == [10, 15, 17, 20, 22, 23, 25, 26, 28, 31, 34, 36, 38, 40, 42, 44, 48, 50, 56, 58, 61, 63, 66, 68, 70, 72, 74, 76, 78, 80, 82, 84, 86, 90, 92, 94, 95, 96, 98, 100, 101, 103, 104, 105, 107, 109, 111, 113, 115, 118, 120, 124, 126, 130, 132, 138, 140, 141, 143, 144, 146, 147, 149, 150, 152, 156, 158, 164, 166, 168, 170, 172, 174, 176, 179, 181, 182, 184, 185, 187, 188, 190, 191, 193, 194, 196, 198, 200, 204, 205, 207])
